        ...
        paginatedData.forEach((item) => {
            ...
            if (isPriorityModeActive) {
                ...
                <tr class="${rowClass}">
                    ...
                    <td class="font-monospace fw-bold">${item.materialNo} ${isItemPriority ? 'â­' : ''}</td>
                ...
                if (isItemPriority) statusText = "â­ PRIORITAS";
    ...
    function renderMainTableRows() {
        ...
        paginatedData.forEach(item => {
            ...
            if (isItemPriority && !isZero && !isCritical) {
                ...
                badgeComponent = `<span class="badge bg-info text-white fw-bold px-2 w-100">â­ PRIORITAS</span>`;
            } else if (isItemPriority) {
                badgeComponent = `<span class="badge ${isZero?'bg-danger':'bg-warning text-dark'} fw-bold px-2 w-100">â­ PRIORITAS</span>`;
            }

            htmlBuffer += `
            <tr class="${rowStyleClass}">
                <td class="font-monospace fw-bold text-secondary text-nowrap">${item.materialNo} ${isItemPriority ? 'â­' : ''}</td>
